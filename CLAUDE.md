...
**이 규칙들은 선택사항이 아닙니다. MUST FOLLOW.**

모든 작업은 반드시 이 순서를 따라야 합니다:
1. **STOP**: 작업 시작 전 체크리스트 확인
2. **TEST FIRST**: 테스트부터 작성 (예외 없음)
3. **IMPLEMENT**: 테스트 통과를 위한 최소 구현
4. **VERIFY**: 80% 커버리지 확인
5. **COMMIT**: Git 커밋 컨벤션 준수
